...
class VectorQuantizer(nn.Module):
    def __init__(self, n_e, e_dim, beta, entropy_loss_ratio, l2_norm, show_usage, num_clusters, cluster_dim):
        super().__init__()
        self.n_e = n_e
        self.e_dim = e_dim
        self.beta = beta
        ...
        assert n_e * e_dim % cluster_dim == 0
        code_length = n_e * e_dim // cluster_dim
        self.codebook = nn.Parameter(torch.empty((num_clusters, cluster_dim)))
        codes = torch.empty((code_length,), dtype=torch.int32)
        self.register_buffer('codes', codes)
        if self.show_usage:
            self.register_buffer("codebook_used", nn.Parameter(torch.zeros(65536)))

    def embedding(self):
        emb = self.codebook[self.codes].view(-1, self.e_dim)
        return emb
